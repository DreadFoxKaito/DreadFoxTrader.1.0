#!/usr/bin/env python3
"""
Dreadfox.Superhexagon.Schwab.py (Web-App Compatible)

Clone of the Superhexagon.Robinhood strategy using Schwab Trader + Market Data APIs.

Preserved behaviors:
- Indicator logic (MA20/MA78/MA190, RSI, RSI derivative)
- "Hexagram" buy constraint: do not buy if ticker holdings exceed 1/portfolio_cap_divisor of portfolio
- Trailing-stop sell for indicator-driven sells (2x ATR) in regular hours
- Stop-loss arming/trigger logic (with disarm fixes)
- Portfolio summary table printed each loop
- Infinite loop with user-defined sleep duration (params.json)
...
TIMEFRAMES: Dict[str, Dict[str, Any]] = {
    "1m": {"periodType": "day", "period": 10, "frequencyType": "minute", "frequency": 1, "fallbacks": []},
    "5m": {"periodType": "day", "period": 10, "frequencyType": "minute", "frequency": 5, "fallbacks": ["1m"]},
    "10m": {"periodType": "day", "period": 10, "frequencyType": "minute", "frequency": 10, "fallbacks": ["5m", "1m"]},
    "15m": {
        "periodType": "day",
        "period": 10,
    ...
    "1d": {"periodType": "year", "period": 1, "frequencyType": "daily", "frequency": 1, "fallbacks": []},
}
...
def get_portfolio_value(account: Dict[str, Any]) -> float:
    return float(_pick_equity(account))


def get_stock_position(ticker: str, positions: List[Dict[str, Any]]) -> Tuple[float, float]:
    for p in positions:
        inst = p.get("instrument") or {}
        ...
        return float(qty), float(avg_buy)
    return 0.0, 0.0


def get_ticker_holding_value(
    ticker: str, *, positions: List[Dict[str, Any]], current_price: float
) -> float:
    qty, _avg_buy = get_stock_position(ticker, positions)
    return float(qty * current_price)
...
def _extract_hlc(hist: List[Dict[str, Any]]) -> Tuple[List[float], List[float], List[float]]:
    """
    ...
    for row in hist:
        ...
        # Fallback to Robinhood format if needed
        if hp is None or hp in ("None", ""):
            hp = row.get("high_price")
        if lp is None or lp in ("None", ""):
            lp = row.get("low_price")
        if cp is None or cp in ("None", ""):
            cp = row.get("close_price")

        # Skip if any values are still missing/invalid
...
def should_buy(
    ...
    *,
    ...
    if None in (ma20, ma78, ma150, rsi, rsi_derivative):
        return False

    divisor = max(2, int(portfolio_cap_divisor))
    ticker_value = get_ticker_holding_value(
        ticker, positions=positions, current_price=current_price
    )
    ...
    if portfolio_cap_mode == "percent":
        ...
        if ticker_value > ticker_cap_value:
            ...
            return False
    elif portfolio_value > 0 and ticker_value > (portfolio_value / divisor):
        print(
            f"Cannot buy {ticker}. Current holdings ${ticker_value:.2f} exceed "
    ...
    return (
        ...
        and rsi < BUY_RSI_HIGH
        and rsi > BUY_RSI_LOW
        and rsi_derivative > BUY_RSI_D_MIN
    )
...
def should_sell(
    ...
    *,
    ...
    rsi = calculate_rsi(prices)
    rsi_derivative = _rsi_derivative(prices)
    quantity, avg_buy_price = get_stock_position(ticker, positions)
    ...
    return (
        ...
        and current_price > ma78
        and current_price > ma150
        and rsi_derivative < SELL_RSI_D_MAX
    ...
    *,
    ...
    if stoploss_state[ticker]["armed"]:
        ...
        if current_price <= stoploss_trigger_price:
            quantity, _ = get_stock_position(ticker, positions)
            ...
            try:
                ...
                if session_state == "extended":
                    ...
                    resp = place_limit_sell(ticker, float(sell_qty), session_tag, limit_price)
                elif session_state == "regular":
                    resp = place_market_sell(ticker, float(sell_qty), session_tag)
                else:
                ...
                print(f"Order state: status={resp.status_code}. Sell might not have executed.")
...
def print_indicator_signals(
    ...
    *,
    ...
    if sell_signal is True:
        ...
        action = "Buy"
    elif None not in (ma20, ma78, ma150, rsi, rsi_derivative):
...
def print_portfolio_summary(
    *,
    ...
    portfolio_value = get_portfolio_value(account)
    ...
    for p in positions:
        ...
        if sym:
            symbols.append(sym)
    quotes = get_quotes_map(sorted(set(symbols)))
    ...
    for p in positions:
        inst = p.get("instrument") or {}
        ticker = str(inst.get("symbol") or "").upper().strip()
        if not ticker:
            continue
        quantity = _safe_float(p.get("longQuantity"), default=0.0)
        if quantity <= 0:
            quantity = _safe_float(p.get("quantity"), default=0.0)
        ...
        qobj = _get_quote_for_symbol(quotes, ticker)
        current_price = _price_from_quote(qobj, prefer_extended=False)
        ...
        percentage_gain_loss = (
            ((current_price - avg_buy_price) / avg_buy_price) * 100 if avg_buy_price > 0 else 0
        )

        print(
            f"{ticker:<10}{quantity:<10.2f}{portfolio_percentage:<15.2f}{avg_buy_price:<15.2f}{percentage_gain_loss:<15.2f}"
        )
...
def _effective_timeframe_key(timeframe_key: str, session_state: str) -> str:
    if timeframe_key == "1h":
        key = "30m"
    else:
        key = timeframe_key
...
def main_trading_loop(
    *,
    ...
    portfolio_value = get_portfolio_value(account)
    available_cash = _pick_available_cash(account)
    buying_power = _pick_buying_power(account)
    divisor = max(2, int(portfolio_cap_divisor))
    cap_pct = None if portfolio_cap_mode == "percent" else (100.0 / divisor if divisor > 0 else 0.0)

    quotes = get_quotes_map(tickers)
    ...
    for ticker in tickers:
        try:
            ...
            base_key = _effective_timeframe_key(timeframe, session_state)
            tf_chain = _resolve_timeframe_chain(base_key)
            ...
            if (len(closes) + 1) < MIN_REQUIRED_CANDLES:
                print(
                    f"[{ticker}] Requested {MIN_REQUIRED_CANDLES}+ candles for timeframe "
                    f"{base_key}; received {len(closes)}."
                )
            ...
            sell_signal = should_sell(ticker, prices, positions=positions)
            ...
            try:
                ...
                if can_place_order:
                    if session_state == "extended":
                        ...
                        print(f"[{ticker}] BUY signal (extended hours) -> placing midpoint limit buy for {num_shares} shares at ${float(mid_price):.2f}...")
                        resp = place_limit_buy(ticker, float(num_shares), session_tag, limit_price)
                    else:
                        resp = place_market_buy(ticker, float(num_shares), session_tag)
                else:
                    if session_state == "extended":
                        print(f"[{ticker}] Extended hours detected but allow_extended_hours_orders=False; skipping buy order.")
                    else:
                        print(f"[{ticker}] Market closed; skipping buy order.")
        ...
        # Sell logic (indicator-driven trailing stop)
            ...
            try:
                ...
                if can_place_order:
                    if session_state == "extended":
                        limit_price = mid_price
                        print(f"[{ticker}] SELL signal (extended hours) -> placing midpoint limit sell for {num_shares} shares at ${float(mid_price):.2f}...")
                        resp = place_limit_sell(ticker, float(num_shares), session_tag, limit_price)
                        trades_made.append(
                            f"Sold {num_shares} shares of {ticker} with limit (price=${limit_price:.2f})."
                        )
                    else:
                        if atr is None:
                            ...
                            trail_amount = max(MIN_TRAIL_AMOUNT_USD, ATR_MULTIPLIER * atr)
                    ...
                    if session_state == "extended":
                        print(f"[{ticker}] Extended hours detected but allow_extended_hours_orders=False; skipping sell order.")
                    else:
                        print(f"[{ticker}] Market closed; skipping sell order.")
...
def main() -> int:
    ...
    def write_status(payload: Dict[str, Any]) -> None:
        ...
        payload["ts"] = iso_now()
        payload["script"] = "Superhexagon.Schwab"
    ...
    num_shares = int(params.get("num_shares", 1))
    ...
    print("Rules: BUY RSI_d > 0.25 (MA20/78/190, RSI 30-55); SELL RSI_d < 0.5 (RSI > 69, price > MAs).")
...
"""
            ...
            base_key = _effective_timeframe_key(timeframe, session_state)
            tf = TIMEFRAMES.get(base_key, TIMEFRAMES["30m"])
            print(
                f"Using timeframe: {timeframe} "
                f"(periodType={tf['periodType']}, frequencyType={tf['frequencyType']}, frequency={tf['frequency']})"
            )
            time.sleep(max(0.0, sleep_duration))

        except KeyboardInterrupt:
            print("\nExiting Superhexagon loop.")
            return 0
        except Exception as e:
