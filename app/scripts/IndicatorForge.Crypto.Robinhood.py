    ...
    while True:
        ...
        for symbol in symbols:
            try:
                ...
                hist = get_crypto_historicals(symbol, interval, span, _db_path=db_path, _connection_id=connection_id)
                ...
                for row in hist:
                    try:
                        close_val = float(row.get("close_price"))
                    except Exception:
                        continue
                    prev_close = closes[-1] if closes else close_val
                    try:
                        open_val = float(row.get("open_price"))
                        high_val = float(row.get("high_price"))
                        low_val = float(row.get("low_price"))
                    except Exception:
                        open_val = prev_close
                        high_val = max(prev_close, close_val)
                        low_val = min(prev_close, close_val)
                    opens.append(float(open_val))
                    highs.append(float(high_val))
                    lows.append(float(low_val))
                    closes.append(float(close_val))
                if len(closes) < rule_min_candles:
                    _log_warn(f"[{symbol}] Not enough historical candles.")
                    tickers_status.append({"symbol": symbol, "signal": "NO_DATA"})
                    continue
                prev_close = float(closes[-1])
                current_price_f = float(current_price)
                opens.append(prev_close)
                highs.append(max(prev_close, current_price_f))
                lows.append(min(prev_close, current_price_f))
                closes.append(current_price_f)
