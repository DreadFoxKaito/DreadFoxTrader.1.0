        ...
        for symbol in symbols:
            try:
                ...
                for tf_key in fetch_timeframes:
                    ...
                    tf_highs: List[float] = []
                    tf_lows: List[float] = []
                    tf_closes: List[float] = []
                    for row in hist_tf:
                        try:
                            close_val = float(row.get("close_price"))
                        except Exception:
                            continue
                        prev_close = tf_closes[-1] if tf_closes else close_val
                        try:
                        ...
                        tf_opens.append(float(open_val))
                        tf_highs.append(float(high_val))
                        tf_lows.append(float(low_val))
                        tf_closes.append(float(close_val))
